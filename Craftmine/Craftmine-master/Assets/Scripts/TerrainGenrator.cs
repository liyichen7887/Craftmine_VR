...
public class TerrainGenrator : MonoBehaviour {
    ...
    GameObject generateChunk(int x, int z)
    {
        ...
        if(chest_chunck.x == x && chest_chunck.y == z)
        {
            chunks[x, z].GetComponent<TerrainChunkGenerator>().chest_generation = true;
            Debug.Log("Assign chest chunk");
        }
